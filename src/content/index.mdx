...
1. **`xfwat_fwd_measure_adj`**: Forward simulation, adjoint source measurements and computing of adjoint wavefields.
2. **`xfwat_post_proc`**: Taking sum of kernels, preconditioning, smoothing and weight sum of gradient for joint inversion.
3. **`xfwat_optimize`**: Optimization of model parameters based on steepest descent, conjugate gradient or L-BFGS methods.
